...
import { TOOLS, TOOL_BY_SLUG, relatedTools, type ToolSlug } from "@/lib/content/tools";
...
export default async function ToolMarketingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const trustSignals = isFree
    ? ["100% free", "No signup required", "No credit card", "Instant results"]
    : [
  ...
  return (
    <>
      ...
      <Hero
        eyebrow={isFree ? "Free forever" : `${tool.name}`}
        title={tool.heroTitle}
        tldr={content.tldr}
        primaryCta={{
          href: isFree ? "/signin" : "/signin",
          label: isFree ? "Use it free" : "Try it free",
        }}
      ...
      <Section>
        <SectionHeader eyebrow="Features" title={`What makes Craftly's ${tool.name} different`} />
        <div className="mt-12">
      ...
      <Section className="bg-secondary/30">
        <SectionHeader
          eyebrow="Comparison"
          title={`Craftly vs the alternatives`}
